...
6. **Pick formulas.** For each feature, identify which formula(s) from `bd formula list` will be poured, and the variable bindings. If no formula fits, note "needs new formula" — escalate to the user before `/compose` runs (this is a *workflow* gap, not a tech preference).
