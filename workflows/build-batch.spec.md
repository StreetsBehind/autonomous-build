...
- **Startup recovery + stale-claim reaper**: before the first dispatch, the orchestrator (a) reads the checkpoint and seeds `mergedSet/blockedSet/failedSet` so processed beads aren't redone, and (b) reaps stale claims — at startup nothing has been dispatched yet, so **every `in_progress` bead is necessarily an orphan from a crashed run**. For each orphan the reaper decides salvage-vs-reset:
  - **Already-merged salvage (execute-2)**: the merge agent's `bd close` is its *last* step, so a crash between `git merge --no-ff` and `bd close` leaves a bead `in_progress` whose work is **already on `main`**. Reaping it back to `open` would rebuild merged work (and a deterministic crash could permanently strand it). So before reopening, if `bead/<id>` is already an ancestor of `main` (`git merge-base --is-ancestor bead/<id> main`), the reaper just `bd close`s it (+ removes the worktree and the now-merged branch) and records it under `closedAlreadyMerged` — counted as merged, never rebuilt. Local refs only.
  - **Reset (genuinely unmerged)**: otherwise reset to `open` (un-claimed), `bd worktree remove --force` the worktree, and delete the stale `bead/<id>` branch so it re-dispatches cleanly. Rebuilding from scratch is the safe choice: an unmerged worker commit never passed the post-merge gate on `main`. The count of orphans whose unmerged work was discarded is logged (T7).
  - **Crash-loop quarantine (execute-1)**: a bead that hard-crashes the runtime mid-worker (OOM, node restart) would otherwise loop reap→repick→recrash forever, silently burning the window (this is narrow to true *orchestrator-level* crashes — worker-*reported* failures already block on first occurrence). On each reset the reaper increments a durable `metadata.reaped` counter (read-merge-write so `filesTouched` etc. are preserved — there is no resident controller to hold an in-memory window); once it reaches `REAP_QUARANTINE_LIMIT` (3) the orphan is set `blocked` with a crash-loop note (recorded under `quarantined`, added to `blockedSet`) and routed to `/escalate` instead of reopened.
