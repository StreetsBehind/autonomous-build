...
3. If `planSource=='lock'`: validate `schemaVersion == 1` (current). Fail loud on version mismatch — this workflow only understands schema 1.
...
6. For each formula referenced in `lock.featureOrder[].formulas` (or parsed from plan.md): `bd formula list | grep <name>` must match. If any are missing, fail with the list (T1: do not guess).
