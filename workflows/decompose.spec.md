...
**Output per agent:** `PourResult = { feature: <name>, pourRoot: <id> | "<dry-run>", children: [{id, title, metadata}], expectedStepTitles: [<title>], status: 'ok' | 'failed', error?: <message> }`

**Post-pour step-completeness assertion (decompose-2, deterministic).** `bd mol pour` can exit 0 yet spawn only a *subset* of a formula's declared steps (a bd hiccup, a jsonl write race) — `pourFailed` stays 0 and the deficit only *maybe* surfaces in the probabilistic Phase 6 verifiers. Each pour agent therefore returns `expectedStepTitles`: the step titles it is **confident** should have poured as children, after variable substitution and honoring each step's `condition`/`needs` (omit-if-unsure — a wrongly-included title would force a false NEEDS-FIX, so the list is conservative and can only catch genuine absences). The orchestrator then asserts (pure JS, normalized title compare) that every expected title resolved to a real child; any `pourStepDeficits` forces **NEEDS-FIX**. Skipped on dry-run (children are placeholders) and when the expected list is empty.
