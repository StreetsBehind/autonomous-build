...
### `nfrs` (optional, may be empty or omitted)
Non-functional requirements with **measurable** targets, lifted from `vision.md` §6 (performance, security, privacy, compliance, data-residency, availability, …). Each entry is `{id, category, statement, target, verify}`. This field exists because §6 constraints like "data stays in my region" used to route only to advisory `tenets.md` prose — a sentence, never a bead with an acceptance criterion and a test, never scored, never gated. NFRs are **first-class**: unlike `concerns[]` (decide-and-cite), `/decompose` pours a dedicated **enforcement bead** per NFR (AC derived from `target`, test seeded from `verify`) and **gates** that every NFR maps to ≥1 enforcement bead — a NFR with no bead is NEEDS-FIX. Kept optional so adding it does not force a `schemaVersion` bump; `/vision` populates it from §6. See bead `autonomous-build-lbq.16`.
